...
Some processing functions for computations
"""
import torch
...
def normalize2(L,Lx, half_interval = False):
    ...
    M = L.shape[0]
    assert(M == L.shape[1])
    topeig = spl.eigsh(L, k=1, which="LM", return_eigenvectors = False)[0]    # we use the maximal eigenvalue of L to normalize the lower and upper Laplacian
